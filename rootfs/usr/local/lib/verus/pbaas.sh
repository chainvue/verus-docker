...
resolve_root_rpc() {
	...
	[[ "${ROOT_CHAIN:-VRSC}" == "VRSC" ]] || default_port="$VRSCTEST_DEFAULT_RPC"
	[[ "${ROOT_CHAIN:-VRSC}" == "VRSC" ]] || root_conf="${VERUS_HOME}/.komodo/vrsctest/vrsctest.conf"
	...
	if [[ -z "$ROOT_RPC_HOST_RESOLVED" && -f "$root_conf" ]]; then
		log_info "using the local ${ROOT_CHAIN:-VRSC} config as the root RPC source"
	...
	log_info "root chain RPC: http://${ROOT_RPC_HOST_RESOLVED}:${ROOT_RPC_PORT_RESOLVED} (chain ${ROOT_CHAIN:-VRSC})"
}
...
write_root_conf_shim() {
	...
	[[ "${ROOT_CHAIN:-VRSC}" == "VRSC" ]] ||
		root_conf="${VERUS_HOME}/.komodo/vrsctest/vrsctest.conf"

	if [[ -f "$root_conf" && -z "${ROOT_RPC_HOST:-}${ROOT_RPC_URL:-}" ]]; then
		log_debug "root config already present at ${root_conf}; leaving it alone"
		return 0
	fi
...
# No ${ROOT_CHAIN:-VRSC} daemon runs in this container. verusd reads these values
